"""Klasa realizjujaca metode stycznych"""
...
class Styczne:
    ...
    def iteruj(self, iteracje, wyswietlaj = 0):
        ...
        # sprawdzam, ktory z punktow powinien byc punktem startowym
        # - oznaczam go x_akt
        ...
        for k in range(iteracje):
            # obliczam wartosc funkcji f i pochodnej w punkcie x_akt
            wart_x = self.funkcja.wartosc(x_akt)
            poch_x = self.pochodna1.wartosc(x_akt)
            if poch_x == 0:
                print("Nie mozna zastosowac metody.")
                return 0
            x_akt -= wart_x / poch_x 
