"""Klasa realizjujaca metode parabol"""
...
class MetodaParabol:
    ...
    def calkuj(self, n):
        ...
        h = (self.b-self.a) / (2.0*n)
        calka = self.f.wartosc(self.a) + self.f.wartosc(self.b)
        calka += 4*self.f.wartosc(self.a+h)
        for i in range(1, n):
            calka += 2*self.f.wartosc(self.a+2*i*h)
            calka += 4*self.f.wartosc(self.a+(2*i+1)*h)
        return h*calka/3
